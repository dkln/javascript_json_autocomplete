/**
 * JSON Javascript autocomplete class
...
Autocomplete.prototype = {
  ...
  updateMultiVals: function() {
    ...
    for(i = 0; i < vals.length; i++) {
      ...
      if(this.multiselectList.find('li[rel="' + vals[i] + '"]').length == 0) {
        // no, so go request it!
        $.getJSON(this.itemUrl.replace('%25', vals[i]), this.bind(this.handleUpdateMultiValsResponse));
      }
    }
  },
  ...
  handleMultiValItemClick: function() {
    // remove from values
  ...
  filter: function() {
    $.getJSON(this.autocompleteUrl.replace('%25', this.input.val()), this.bind(this.handleFilterResponse));
  },
  ...
  handleFilterResponse: function(data) {
    ...
    if(!data || data.length == 0) {
      ...
      if(data.length == 1 && data[0].name != this.input.val()) {
        this.dropdown.hide();
        window.clearTimeout(this.timer);
